using System.ComponentModel.DataAnnotations.Schema;
using System.Data.Entity.ModelConfiguration;

namespace AAWebSmartHouse.Models.Models.Mapping
{
    public class sensordatabyhourMap : EntityTypeConfiguration<sensordatabyhour>
    {
        public sensordatabyhourMap()
        {
            // Primary Key
            this.HasKey(t => t.SensorDataByHourId);

            // Properties
            this.Property(t => t.SensorValue)
                .IsRequired()
                .HasMaxLength(50);

            // Table & Column Mappings
            this.ToTable("sensordatabyhour", "sql7112984");
            this.Property(t => t.SensorDataByHourId).HasColumnName("SensorDataByHourId");
            this.Property(t => t.SensorId).HasColumnName("SensorId");
            this.Property(t => t.SensorValue).HasColumnName("SensorValue");
            this.Property(t => t.SensorDataDateTime).HasColumnName("SensorDataDateTime");

            // Relationships
            this.HasRequired(t => t.sensor)
                .WithMany(t => t.sensordatabyhours)
                .HasForeignKey(d => d.SensorId);

        }
    }
}
